...
The last three commands also need an agent deployment, which the CLI does not scaffold — you create one once in the web app, following [Configure Runtime Resources](/docs/guides/configure-runtime-resources). Until a deployment exists, stop after `challenge-pack publish`: `eval start` fails with `no deployments found in workspace`.

...
Conversational authoring returns plain-English content: a title, agent instructions, one to three example inputs and success criteria. Go constructs the merged blueprint, case keys, nonempty-output check, single semantic evaluator and score dimensions. The model no longer authors scoring enums, regexes or cross-references. Proposed requirements and assumptions are arrays of strings; provenance/status are server-owned. Assumptions are labeled in the reply and stored as proposed requirements. Instructions tell the assistant to proceed when the task is known or defaults are delegated, use placeholders for unknown business facts, and distinguish an unconnected sample from the user's existing agent. Those conversational quality rules still require live model evaluation; structural validation alone cannot prove grounding.

A correction includes the invalid response as quoted diagnostic data only if the complete request fits. Otherwise the same single correction regenerates from the original request, requirements/provenance and validation error, omitting the invalid response. If that also exceeds the context limit, no correction call is sent. Each response is decoded into a fresh object so partially decoded invalid content cannot leak into a later reply. All example text and criteria are preserved. Explicit imports retain mechanical checks and case-specific expected references; accepted evaluation contracts stay pinned during improvements and retests. The simpler generation contract reduces structural failure modes; it does not prove generated tests are sufficient or correct.

An operator-verified model profile may set `structured_outputs: true` to send authoring with `response_format.type=json_schema` and `strict=true`. The schema and local validation both allow at most three proposed requirements and two assumptions per reply. The complete schema is included in every input bound, including correction preflight. Profiles without verified support retain the bounded JSON-object path; a provider rejection never triggers an automatic schema downgrade. On September 8, the live Dots/AtlasCloud endpoint advertised `structured_outputs` and zero prompt/completion prices; this capability was enabled in the ignored local profile. [OpenRouter documents endpoint-specific support and varying enforcement](https://openrouter.ai/docs/guides/features/structured-outputs), so local parsing/validation remains authoritative. No new library or model framework is used.
...
Expected Vibe faults cross the activity boundary as typed, nonretryable application errors with their safe code/message. The finalizer persists that specific fault instead of relabeling every failure as an interrupted worker. Unknown worker failures retain the generic interruption message. A Temporal version marker preserves the previous finalizer for old histories.
...
A subsequent user-reported failure exposed an oversized correction request and generic Temporal error masking. After the bounded-repair/error fixes, the same local conversation produced a draft successfully; its unaccepted criteria were corrected before evaluation. Its first check then completed all six target/evaluator calls at verified provider cost `0`, with three passed cases, zero failures and zero unknowns. The earlier failed operation's diagnostic was corrected from verified logs without changing its provider evidence, accounting or trial counters. Regression coverage now includes large invalid authoring output, two-call exhaustion, partial decode isolation, typed Temporal failures, legacy finalization and generation-only phrase-check validation.

September 8 validation found further limitations in Dots authoring: wrong requirement types, invalid regex configuration and a repeated proposal-count violation. New drafts now use the semantic contract above, with fixed Go scoring configuration and opt-in verified schema output. The delegated-defaults request and final marketing-copy continuation each completed in one schema-enabled Dots call at reported provider cost `0`. A fresh sparse conversation still over-asked for the original agent prompt before the final coordinator instructions clarified that pending proposals do not block a sample draft. The final continuation succeeded, but this is not a measured reliability cohort or proof that free-model onboarding is consistently good.

One temporary free Gemma comparison failed before returning content. The old generic error mapping lost its precise cause; that attempt remains `UNCERTAIN` and was not repeated or retroactively relabeled. The temporary option was removed. Future typed provider failures retain safe categories, and the shared OpenAI-compatible error parser accepts numeric OpenRouter error codes as well as string codes. Raw provider messages are not exposed, uncertain reservations remain held, and no automatic provider retry was added.

The affected real-PostgreSQL/race/API tests, runtime provider/scoring tests, API/worker builds, vet and focused frontend lint passed. Live browser inspection covered the sample-agent disclosure, proposal labels and compiled draft. The existing trial had already consumed its initial check; no quota was reset and no new live evaluation was run. Evaluation execution, honest UNKNOWN results, independent model roles, immutable retests and canonical saving were exercised with persisted integration fixtures. The local pilot remains on free routes and requires broader model-quality validation before launch.

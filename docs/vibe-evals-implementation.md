...
Authoring uses a compact output example grounded in the merged blueprint types and scoring enums, without embedding a competing task/rules JSON envelope. A correction includes the invalid response as quoted diagnostic data only if the complete request fits. Otherwise the same single correction regenerates from the original request, requirements/provenance and validation error, omitting the invalid response. If that also exceeds the context limit, no correction call is sent. Each response is decoded into a fresh object so partially decoded invalid content cannot leak into a later reply.

New AI-authored semantic previews use a nonempty-output check (`regex_match`, `literal:.+`) plus conditional evaluation criteria. Literal phrase/regex requirements alongside a semantic evaluator are rejected for the bounded correction; case-specific expected-value references remain supported. This authoring restriction does not modify explicit imports or accepted evaluation contracts. It reduces one observed failure mode; it does not prove generated tests are sufficient or correct.

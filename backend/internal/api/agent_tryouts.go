...
func builtinAgentTryoutTemplates() []AgentTryoutTemplate {
	return []AgentTryoutTemplate{
		...
		},
		{
			...
			Description:        "Turn a brief into a real PowerPoint and PDF with slides, charts, and speaker notes.",
			Available:          true,
			...
			Runtime:            json.RawMessage(`{"adapter":"slide_deck_v1","sandbox":{"filesystem":"workspace","shell":"allowed","network":"disabled"},"expected_artifacts":[{"key":"presentation","type":"pptx","path":"deck.pptx"},{"key":"presentation_pdf","type":"pdf","path":"deck.pdf"},{"key":"structured_deck","type":"json","path":"deck.json"}],"instructions":"Deliver real office files the user can open and share. Build deck.pptx with python-pptx: title slide, content slides, and at least one chart or diagram (generate with matplotlib, save under assets/, embed on a slide). Export deck.pdf (LibreOffice: soffice --headless --convert-to pdf deck.pptx). Write deck.json with fields title (string), slides (array of {title, bullets, notes}), charts (array). Do not stop at markdown outlines.","validation":{"validators":[{"key":"has_presentation","type":"artifact_produced","artifact_key":"presentation"},{"key":"has_pdf","type":"artifact_produced","artifact_key":"presentation_pdf"},{"key":"has_title","type":"json_field","field":"title"},{"key":"has_slides","type":"json_field","field":"slides"}],"score_dimensions":["correctness","visuals","reliability","cost"]}}`),
			EvaluationSpec:     json.RawMessage(`{"validators":[{"key":"has_presentation","type":"artifact_produced","artifact_key":"presentation"},{"key":"has_pdf","type":"artifact_produced","artifact_key":"presentation_pdf"},{"key":"has_title","type":"json_field","field":"title"},{"key":"has_slides","type":"json_field","field":"slides"}],"scorecard":{"dimensions":["correctness","visuals","reliability","cost"]}}`),
			DefaultModelPolicy: json.RawMessage(`{"mode":"hosted_default","max_models":1}`),
			AnonymousEnabled:   true,
			MaxInputBytes:      64 * 1024,
